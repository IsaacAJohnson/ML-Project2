...
pr_bank <- prcomp(x = select(train, -Churn), scale = T, center = T)
summary(pr_bank)
...
prc <- bind_cols(select(train,Churn),as.data.frame(pr_bank$x)) %>%
  select(1:6) %>%
...
#kappa =1?
confusionMatrix(predict(fit, train),factor(train$Churn))
